...
BATCH_SIZE = 16
NUM_CLASSES = 101
EXP_NAME = "DeiTBase_food101_100epochs_lr1e-3_step10_gamma0.85"
...
class Dataset(torch.utils.data.Dataset):
	def __init__(self, files, mode, class_to_idx):
		self.files = files
	...
	def __getitem__(self, idx):
		...
		label = self.class_to_idx[Path(file).parent.name]
...
def get_dataset(root: str, batch_size: int=BATCH_SIZE):
	meta_dir = Path(root) / "food-101" / "meta"
	images_dir = Path(root) / "food-101" / "images"

	classes = (meta_dir / "classes.txt").read_text().splitlines()
	class_to_idx = {name: i for i, name in enumerate(classes)}

	train_ids = (meta_dir / "train.txt").read_text().splitlines()
	test_ids = (meta_dir / "test.txt").read_text().splitlines()

	train_files = [str(images_dir / f"{img_id}.jpg") for img_id in train_ids]
	test_files = [str(images_dir / f"{img_id}.jpg") for img_id in test_ids]

	train_dataset = Dataset(train_files, mode="train", class_to_idx=class_to_idx)
	test_dataset = Dataset(test_files, mode="test", class_to_idx=class_to_idx)
...
if __name__ == "__main__":
	...
	model = DeiTBase(num_classes=NUM_CLASSES, pretrained=True, freeze_backbone=True).to(device)
